	...
	for (int localMapId = 0; localMapId < numLocalMaps; ++localMapId)
	{
		...
		for (int entryId = 0; entryId < noTotalEntriesPerLocalMap; entryId++)
		{
			...
			for (int z = 0; z < SDF_BLOCK_SIZE; z++) for (int y = 0; y < SDF_BLOCK_SIZE; y++) for (int x = 0; x < SDF_BLOCK_SIZE; x++)
			{
				...
				int cubeIndex = buildVertListMulti(vertList,vertListColorMap, globalPos, Vector3i(x, y, z), &localVBAs, &hashTables, localMapId);
				...
				for (int i = 0; triangleTable[cubeIndex][i] != -1; i += 3)
				{
					...
					//printf("Triangles: %f %f %f",triangles[noTriangles].p0,triangles[noTriangles].p1,triangles[noTriangles].p2 );
					...
					//printf("TrianglesColor: %f %f %f",triangleColorMap[noTriangles].p0,triangleColorMap[noTriangles].p1,triangleColorMap[noTriangles].p2 );
